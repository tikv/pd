...
func utilTransferRegionCountParseLog(operator, content string, expect []uint64) bool {
	r := GetTransferRegionCounter().CompileRegex(operator)
	results := GetTransferRegionCounter().parseLine(content, r)
...
func (t *testParseLog) TestTransferRegionCountParseLog(c *C) {
	{
		...
		c.Assert(utilTransferRegionCountParseLog(operator, content, expect), Equals, true)
	}
	{
