...
If there is TiDB in cluster, `key-type` should be `table`. If `key-type` is `raw`, `placement rule` cannot work. Whether pd can merge region cross table will be determined by `enable-cross-table-merge`
