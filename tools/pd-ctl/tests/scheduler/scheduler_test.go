...
func (suite *schedulerTestSuite) checkSchedulerConfig(cluster *pdTests.TestCluster) {
	...
	testutil.Eventually(re, func() bool {
		...
		return conf["batch"] == 3. && conf["enable-network-slow-store"] == true
	})
	...
	testutil.Eventually(re, func() bool {
		...
		return conf1["batch"] == 10. && conf1["enable-network-slow-store"] == true
	})
	echo = tests.MustExec(re, cmd, []string{"-u", pdAddr, "scheduler", "config", "evict-slow-store-scheduler", "set", "enable-network-slow-store", "false"}, nil)
	re.Contains(echo, "Success!")
	testutil.Eventually(re, func() bool {
		tests.MustExec(re, cmd, []string{"-u", pdAddr, "scheduler", "config", "evict-slow-store-scheduler"}, &conf1)
		return conf1["batch"] == 10. && conf1["enable-network-slow-store"] == false
	})
}
