...
func TestKeyspaceGroup(t *testing.T) {
	...
	defer cancel()
	tc, err := pdTests.NewTestMSCluster(ctx, 1)
	re.NoError(err)
	defer tc.Destroy()
...
func TestSplitKeyspaceGroup(t *testing.T) {
	...
	tc, err := pdTests.NewTestMSCluster(ctx, 3, func(conf *config.Config, _ string) {
...
func TestExternalAllocNodeWhenStart(t *testing.T) {
	...
	tc, err := pdTests.NewTestMSCluster(ctx, 1, func(conf *config.Config, _ string) {
		conf.Keyspace.PreAlloc = keyspaces
	})
	re.NoError(err)
	defer tc.Destroy()
