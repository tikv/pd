...
func NewShowConfigCommand() *cobra.Command {
	...
	sc.Flags().Bool(flagFromAPIServer, true, "read data from api server rather than micor service")
	return sc
}
...
func NewPlacementRulesCommand() *cobra.Command {
	...
	show.Flags().Bool(flagFromAPIServer, true, "read data from api server rather than micor service")
	load := &cobra.Command{
		Use:   "load",
	...
	load.Flags().Bool(flagFromAPIServer, true, "read data from api server rather than micor service")
	save := &cobra.Command{
		Use:   "save",
	...
	ruleGroupShow.Flags().Bool(flagFromAPIServer, true, "read data from api server rather than micor service")
	...
	ruleBundleGet.Flags().Bool(flagFromAPIServer, true, "read data from api server rather than micor service")
	...
	ruleBundleLoad.Flags().Bool(flagFromAPIServer, true, "read data from api server rather than micor service")
