...
import (
	...
	"github.com/tikv/pd/tools/pd-dev/pd-simulator/simulator/info"
...
// CaseMap is a mapping of the cases to the corresponding to initialize functions.
...
func getStoreNum() int {
	storeNum := config.CaseConfigure.StoreNum
	if storeNum < 3 {
...
func getRegionNum() int {
	regionNum := config.CaseConfigure.RegionNum
	if regionNum <= 0 {
