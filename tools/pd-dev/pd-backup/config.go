...
type Config struct {
	*util.GeneralConfig
	...
	FilePath string `toml:"file" json:"file"`
}
...
func newConfig() *Config {
	...
	cfg.FlagSet = flag.NewFlagSet("pd-backup", flag.ContinueOnError)
	fs := cfg.FlagSet
	cfg.GeneralConfig = util.NewGeneralConfig(fs)
	fs.ParseErrorsWhitelist.UnknownFlags = true

	fs.StringVar(&cfg.FilePath, "file", "backup.json", "backup file path and name")
