...
func TestString(t *testing.T) {
	...
	testcases := []struct {
		...
		{int(filtersLen - 1), "store-state-unhealthy-filter"},
...
func TestCounter(t *testing.T) {
	...
	counter.inc(source, storeStateTombstone, 1)
	counter.inc(target, storeStateTombstone, 2)
	// For source action, we only care about sourceID (1)
