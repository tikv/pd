...
func checkHotWriteRegionScheduleByteRateOnly(re *require.Assertions, enablePlacementRules bool) {
	...
	// |    5     |        1MB       |        0.5MB     |
	// |    6     |        4MB       |        0.5MB     |
	tc.UpdateStorageWrittenBytes(1, 7*units.MiB*utils.StoreHeartBeatReportInterval)
	tc.UpdateStorageWrittenBytes(2, 6*units.MiB*utils.StoreHeartBeatReportInterval)
	tc.UpdateStorageWrittenBytes(3, 7*units.MiB*utils.StoreHeartBeatReportInterval)
	tc.UpdateStorageWrittenBytes(4, 4.1*units.MiB*utils.StoreHeartBeatReportInterval)
	tc.UpdateStorageWrittenBytes(5, 1)
	tc.UpdateStorageWrittenBytes(6, 4*units.MiB*utils.StoreHeartBeatReportInterval)
...
func checkWriteScheduleWithReplica3(re *require.Assertions, enablePlacementRules bool) {
	...
	count := 0
	total := 400
	ticker := time.NewTicker(utils.StoreHeartBeatReportInterval * time.Second)
	defer ticker.Stop()
	for i := 0; i < total; i++ {
		select {
		case <-ticker.C:
...
func checkWriteScheduleWithReplica5(re *require.Assertions, enablePlacementRules bool) {
	...
	updateStore := func() {
		...
		tc.UpdateStorageWrittenBytes(2, 4.5*units.MiB*utils.StoreHeartBeatReportInterval)
		tc.UpdateStorageWrittenBytes(3, 4.5*units.MiB*utils.StoreHeartBeatReportInterval)
		tc.UpdateStorageWrittenBytes(4, 4.5*units.MiB*utils.StoreHeartBeatReportInterval)
		tc.UpdateStorageWrittenBytes(5, 4.5*units.MiB*utils.StoreHeartBeatReportInterval)
		tc.UpdateStorageWrittenBytes(6, 0*units.MiB*utils.StoreHeartBeatReportInterval)
	}
	updateStore()
