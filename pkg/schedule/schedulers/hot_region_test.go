...
var (
	writeSchedulerType config.CheckerSchedulerType = "write"
	readSchedulerType  config.CheckerSchedulerType = "read"
)
...
func init() {
	...
	RegisterScheduler(writeSchedulerType, func(opController *operator.Controller, _ endpoint.ConfigStorage, _ ConfigDecoder, _ ...func(string) error) (Scheduler, error) {
		cfg := initHotRegionScheduleConfig()
		return newHotWriteScheduler(opController, cfg), nil
	})
	RegisterScheduler(readSchedulerType, func(opController *operator.Controller, _ endpoint.ConfigStorage, _ ConfigDecoder, _ ...func(string) error) (Scheduler, error) {
...
func TestUpgrade(t *testing.T) {
	...
	defer cancel()
	// new
	sche, err := CreateScheduler(config.HotRegionScheduler, oc, storage.NewStorageWithMemoryBackend(), ConfigSliceDecoder(config.HotRegionScheduler, nil))
	re.NoError(err)
	...
	sche, err = CreateScheduler(config.HotRegionScheduler, oc, storage.NewStorageWithMemoryBackend(), ConfigJSONDecoder([]byte("null")))
	re.NoError(err)
	hb = sche.(*hotScheduler)
	re.Equal([]string{utils.QueryPriority, utils.BytePriority}, hb.conf.GetReadPriorities())
	re.Equal([]string{utils.QueryPriority, utils.BytePriority}, hb.conf.GetWriteLeaderPriorities())
	...
	sche, err = CreateScheduler(config.HotRegionScheduler, oc, storage.NewStorageWithMemoryBackend(), ConfigJSONDecoder([]byte(config51)))
	re.NoError(err)
	hb = sche.(*hotScheduler)
	...
	sche, err = CreateScheduler(config.HotRegionScheduler, oc, storage.NewStorageWithMemoryBackend(), ConfigJSONDecoder([]byte(config54)))
	re.NoError(err)
	hb = sche.(*hotScheduler)
	re.Equal([]string{utils.QueryPriority, utils.BytePriority}, hb.conf.GetReadPriorities())
	re.Equal([]string{utils.QueryPriority, utils.BytePriority}, hb.conf.GetWriteLeaderPriorities())
...
func TestSplitIfRegionTooHot(t *testing.T) {
	...
	cancel, _, tc, oc := prepareSchedulersTest()
	defer cancel()
	hb, err := CreateScheduler(readSchedulerType, oc, storage.NewStorageWithMemoryBackend(), nil)
	re.NoError(err)
	b := &metapb.Buckets{
		RegionId:   1,
		PeriodInMs: 1000,
	...
	hb, _ = CreateScheduler(writeSchedulerType, oc, storage.NewStorageWithMemoryBackend(), nil)
...
func checkHotWriteRegionPlacement(re *require.Assertions, enablePlacementRules bool) {
	...
	labels := []string{"zone", "host"}
	tc.SetMaxReplicasWithLabel(enablePlacementRules, 3, labels...)
	hb, err := CreateScheduler(writeSchedulerType, oc, storage.NewStorageWithMemoryBackend(), nil)
	re.NoError(err)
	hb.(*hotScheduler).types = []resourceType{writePeer}
...
func TestHotWriteRegionScheduleByteRateOnlyWithTiFlash(t *testing.T) {
	...
	sche, err := CreateScheduler(writeSchedulerType, oc, storage.NewStorageWithMemoryBackend(), nil)
	re.NoError(err)
	hb := sche.(*hotScheduler)
...
func TestHotReadRegionScheduleByteRateOnly(t *testing.T) {
	...
	defer cancel()
	tc.SetClusterVersion(versioninfo.MinSupportedVersion(versioninfo.Version4_0))
	scheduler, err := CreateScheduler(readSchedulerType, oc, storage.NewStorageWithMemoryBackend(), nil)
	re.NoError(err)
...
func checkHotReadPeerSchedule(re *require.Assertions, enablePlacementRules bool) {
	...
	sche, err := CreateScheduler(readSchedulerType, oc, storage.NewStorageWithMemoryBackend(), ConfigJSONDecoder([]byte("null")))
	re.NoError(err)
	hb := sche.(*hotScheduler)
	hb.conf.ReadPriorities = []string{utils.BytePriority, utils.KeyPriority}
...
func TestCompatibilityConfig(t *testing.T) {
	...
	hb, err := CreateScheduler(config.HotRegionScheduler, oc, storage.NewStorageWithMemoryBackend(), ConfigSliceDecoder(config.HotRegionScheduler, nil))
	re.NoError(err)
	checkPriority(re, hb.(*hotScheduler), tc, [3][2]int{
		{utils.QueryDim, utils.ByteDim},
		{utils.QueryDim, utils.ByteDim},
	...
	hb, err = CreateScheduler(config.HotRegionScheduler, oc, storage.NewStorageWithMemoryBackend(),
		ConfigSliceDecoder(config.HotRegionScheduler, []string{"read-priorities=byte,query"}))
	re.NoError(err)
	checkPriority(re, hb.(*hotScheduler), tc, [3][2]int{
		{utils.QueryDim, utils.ByteDim},
		{utils.QueryDim, utils.ByteDim},
	...
	re.NoError(err)
	err = storage.SaveSchedulerConfig(config.HotRegionScheduler.String(), data)
	re.NoError(err)
	hb, err = CreateScheduler(config.HotRegionScheduler, oc, storage, ConfigJSONDecoder(data))
	re.NoError(err)
	checkPriority(re, hb.(*hotScheduler), tc, [3][2]int{
		{utils.ByteDim, utils.KeyDim},
		{utils.KeyDim, utils.ByteDim},
