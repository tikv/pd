...
func TestEvictLeader(t *testing.T) {
	...
	sl, err := CreateScheduler(config.EvictLeaderScheduler, oc, storage.NewStorageWithMemoryBackend(), ConfigSliceDecoder(config.EvictLeaderScheduler, []string{"1"}), func(string) error { return nil })
	re.NoError(err)
