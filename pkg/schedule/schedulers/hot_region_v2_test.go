...
func TestHotWriteRegionScheduleWithRevertRegionsDimSecond(t *testing.T) {
	...
	defer cancel()
	sche, err := CreateScheduler(writeSchedulerName, oc, storage.NewStorageWithMemoryBackend(), nil, nil)
	re.NoError(err)
...
func TestHotReadRegionScheduleWithRevertRegionsDimSecond(t *testing.T) {
	...
	cancel, _, tc, oc := prepareSchedulersTest()
	defer cancel()
	sche, err := CreateScheduler(readSchedulerName, oc, storage.NewStorageWithMemoryBackend(), nil, nil)
	re.NoError(err)
	hb := sche.(*hotScheduler)
	hb.conf.SetDstToleranceRatio(0.0)
	hb.conf.SetSrcToleranceRatio(0.0)
...
func TestSkipUniformStore(t *testing.T) {
	...
	cancel, _, tc, oc := prepareSchedulersTest()
	defer cancel()
	hb, err := CreateScheduler(readSchedulerName, oc, storage.NewStorageWithMemoryBackend(), nil, nil)
	re.NoError(err)
