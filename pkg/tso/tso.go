...
// getStorageTimeout returns the timeout for storage operations.
// The returned timeout ensures that the storage timeout is at least
// `config.DefaultTSOSaveInterval - time.Second`, which gives enough buffer
// for storage operations to complete before hitting the save deadline.
func (t *timestampOracle) getStorageTimeout() time.Duration {
	timeout := t.saveInterval
	switch {
	case timeout >= config.DefaultTSOSaveInterval:
...
func (t *timestampOracle) setTSOPhysical(next time.Time, force bool) {
	...
	if (t.isInitializedLocked() || force) &&
		typeutil.SubTSOPhysicalByWallClock(next, t.tsoMux.physical) > 0 {
...
func (t *timestampOracle) getLastSavedTime() time.Time {
	switch last := t.lastSavedTime.Load().(type) {
	case time.Time:
		return last
	default:
...
func (t *timestampOracle) resetUserTimestamp(tso uint64, ignoreSmaller, skipUpperBoundCheck bool) error {
	...
	case physicalDifference == 0 && logicalDifference <= 0:
	...
	case !skipUpperBoundCheck && physicalDifference >= t.maxResetTSGap().Milliseconds():
