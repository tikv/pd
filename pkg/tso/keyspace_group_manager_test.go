...
func (suite *keyspaceGroupManagerTestSuite) TestUpdateKeyspaceGroupMembershipWithUnorderedOldGroup() {
	re := suite.Require()

	groupID := uint32(2)
	oldKeyspaces := []uint32{5, 4, 3} // intentionally unordered
	oldLookupTable := map[uint32]struct{}{
		5: {},
		3: {},
		4: {},
	}
	oldGroup := &endpoint.KeyspaceGroup{
		ID:                  groupID,
		Keyspaces:           append([]uint32{}, oldKeyspaces...),
		KeyspaceLookupTable: oldLookupTable,
	}

	newKeyspaces := []uint32{3, 4, 5}
	newGroup := &endpoint.KeyspaceGroup{
		ID:        groupID,
		Keyspaces: append([]uint32{}, newKeyspaces...),
	}
	...
	// Verify newGroup has the correct sorted keyspaces and lookup table
	verifyLocalKeyspaceLookupTable(re, newGroup.KeyspaceLookupTable, []uint32{3, 4, 5})
	verifyGlobalKeyspaceLookupTable(re, kgm.keyspaceLookupTable, newGroup.KeyspaceLookupTable)
	re.Equal([]uint32{3, 4, 5}, newGroup.Keyspaces)

	// oldGroup should also end up sorted after the update to keep subsequent diffs stable
	re.Equal([]uint32{3, 4, 5}, oldGroup.Keyspaces)
}
