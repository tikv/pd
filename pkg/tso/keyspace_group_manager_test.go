...
func (suite *keyspaceGroupManagerTestSuite) TestGetKeyspaceGroupMetaWithCheck() {
	...
	re.NotNil(allocator)
	re.NotNil(kg)
	allocator, kg, kgid, err = mgr.getKeyspaceGroupMetaWithCheck(1, 0, mgr)
	re.NoError(err)
	re.Equal(constant.DefaultKeyspaceGroupID, kgid)
	re.NotNil(allocator)
	re.NotNil(kg)
	allocator, kg, kgid, err = mgr.getKeyspaceGroupMetaWithCheck(2, 0, mgr)
	re.NoError(err)
	re.Equal(constant.DefaultKeyspaceGroupID, kgid)
	re.NotNil(allocator)
	re.NotNil(kg)
...
func (suite *keyspaceGroupManagerTestSuite) TestDefaultMembershipRestriction() {
	...
	if kerneltype.IsNextGen() {
		...
		allocator, kg, kgid, err = mgr.getKeyspaceGroupMetaWithCheck(constant.DefaultKeyspaceID, 3, mgr)
		re.NoError(err)
...
func (suite *keyspaceGroupManagerTestSuite) TestKeyspaceMovementConsistency() {
	...
	allocator, kg, kgid, err = mgr.getKeyspaceGroupMetaWithCheck(10, constant.DefaultKeyspaceGroupID, mgr)
	re.NoError(err)
	re.Equal(constant.DefaultKeyspaceGroupID, kgid)
	re.NotNil(allocator)
	re.NotNil(kg)
	...
	allocator, kg, kgid, err = mgr.getKeyspaceGroupMetaWithCheck(10, 1, mgr)
	re.NoError(err)
