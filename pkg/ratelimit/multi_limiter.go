...
// NewMultiLimiter returns a global limiter which can be updated in the later.
...
func (l *MultiLimiter) Allow(label string) (DoneFunc, error) {
	var ok bool
...
func (l *MultiLimiter) Update(label string, opts ...Option) UpdateStatus {
...
func (l *MultiLimiter) GetQPSLimiterStatus(label string) (limit rate.Limit, burst int) {
...
func (l *MultiLimiter) GetConcurrencyLimiterStatus(label string) (limit uint64, current uint64) {
	if limit, exist := l.limiters.Load(label); exist {
...
func (l *MultiLimiter) IsInAllowList(label string) bool {
