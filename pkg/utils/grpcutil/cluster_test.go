...
func (suite *clusterSuite) TestQueryRegion() {
	re := suite.Require()
	rc := core.NewBasicCluster()
	rc.PutRegion(core.NewTestRegionInfo(1, 1, []byte("a"), []byte("d")))
	rc.PutRegion(core.NewTestRegionInfo(2, 1, []byte("d"), []byte("g")))
	...
	for _, isFollower := range []bool{true, false} {
		suite.Run("isFollower="+strconv.FormatBool(isFollower), func() {
			resp := QueryRegion(rc, request, isFollower)
			re.NotNil(resp)
			if isFollower {
				re.Equal(pdpb.ErrorType_REGION_NOT_FOUND, resp.GetHeader().GetError().GetType())
			} else {
			...
			re.Equal([]uint64{1, 0}, resp.GetKeyIdMap())
			re.Equal([]uint64{1, 0}, resp.GetPrevKeyIdMap())
			re.NotNil(resp.GetRegionsById()[1].GetRegion())
			re.Nil(resp.GetRegionsById()[3])
		})
	}

	resp := QueryRegion(rc, &pdpb.QueryRegionRequest{
		Keys:     [][]byte{[]byte("c")},
		PrevKeys: [][]byte{[]byte("d")},
		Ids:      []uint64{1},
	}, true)
	re.Nil(resp.GetHeader().GetError())
}
