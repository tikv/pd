...
func TestExitWatch(t *testing.T) {
	...
	checkExitWatch(t, leaderKey, func(server *embed.Etcd, client *clientv3.Client) func() {
		...
		client2, err := etcdutil.CreateEtcdClient(nil, etcd2.Config().ListenClientUrls, etcdutil.DefaultEtcdClientUsager, true)
		re.NoError(err)
...
func checkExitWatch(t *testing.T, leaderKey string, injectFunc func(server *embed.Etcd, client *clientv3.Client) func()) {
	...
	defer clean()
	client2, err := etcdutil.CreateEtcdClient(nil, servers[0].Config().ListenClientUrls, etcdutil.DefaultEtcdClientUsager, true)
	re.NoError(err)
	defer client2.Close()
