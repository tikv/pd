...
| `pd-ci-flaky-triage` | Triage recent PD CI failures from raw logs, review likely flaky tests, and create or update flaky issue actions. | `gh` CLI authenticated with tikv/pd repo access; Python 3 in PATH. | [`.agents/skills/pd-ci-flaky-triage/SKILL.md`](.agents/skills/pd-ci-flaky-triage/SKILL.md) |
