...
| `propagate-pd-downstream-deps` | After a merged PD PR, decide whether exported PD client behavior changed in a downstream-visible way. Skip typo, test-only, comment-only, and refactor-only changes; when propagation is required, bump `github.com/tikv/pd/client` or related PD modules in downstream repos such as `tikv/client-go` and `pingcap/tidb`, run targeted verification, resolve downstream PR conflicts, and keep the PRs merge-ready. | `gh` CLI authenticated for tikv/pd, tikv/client-go, and pingcap/tidb; local clones or permission to clone/push downstream branches. | [`.agents/skills/propagate-pd-downstream-deps/SKILL.md`](.agents/skills/propagate-pd-downstream-deps/SKILL.md) |
