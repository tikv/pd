...
| `add-metrics` | Checklist and guidelines for adding or changing Prometheus metrics — cache WithLabelValues, hot path awareness, cleanup, cardinality, naming, MustRegister, backward compatibility. | None. | [`.agents/skills/add-metrics/SKILL.md`](.agents/skills/add-metrics/SKILL.md) |
| `update-dashboard` | Update the `pingcap/tidb-dashboard` dependency to the latest commit. Bumps `go.mod`/`go.sum` across root, `tests/integrations`, and `tools` modules, verifies the build, and commits. | Go toolchain and network access to `proxy.golang.org`; clean working tree. | [`.agents/skills/update-dashboard/SKILL.md`](.agents/skills/update-dashboard/SKILL.md) |
