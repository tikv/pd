...
|---|---|---|---|
| `apply-pd-go-best-practices` | Compare Go changes against PD-specific coding conventions during targeted review or refactoring when repository-local guidance is needed beyond `make check`. | None beyond the repository's existing Go workflow and local contribution rules. | [`.agents/skills/apply-pd-go-best-practices/SKILL.md`](.agents/skills/apply-pd-go-best-practices/SKILL.md) |
