...
func (h *memberHandler) getMembers() (*pdpb.GetMembersResponse, error) {
	...
	for _, m := range members.GetMembers() {
		...
		if e != nil {
			log.Error("failed to load binary version", zap.Uint64("member", m.GetMemberId()), zap.Error(errs.ErrStorageEtcdLoad.FastGenByArgs()))
		}
		m.BinaryVersion = binaryVersion
		deployPath, e := h.svr.GetMember().GetMemberDeployPath(m.GetMemberId())
		if e != nil {
			log.Error("failed to load deploy path", zap.Uint64("member", m.GetMemberId()), zap.Error(errs.ErrStorageEtcdLoad.FastGenByArgs()))
		}
		...
		if e != nil {
			log.Error("failed to load leader priority", zap.Uint64("member", m.GetMemberId()), zap.Error(errs.ErrStorageEtcdLoad.FastGenByArgs()))
			continue
		}
		m.LeaderPriority = int32(leaderPriority)
		gitHash, e := h.svr.GetMember().GetMemberGitHash(m.GetMemberId())
		if e != nil {
			log.Error("failed to load git hash", zap.Uint64("member", m.GetMemberId()), zap.Error(errs.ErrStorageEtcdLoad.FastGenByArgs()))
			continue
		}
		m.GitHash = gitHash
	}
