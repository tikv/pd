...
func TestSetRegion(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		regions.UpdateSubTree(NewUpdateSubtreeTask(region, origin, overlaps, rangeChanged))
	}
...
func BenchmarkAddRegion(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		regions.UpdateSubTree(NewUpdateSubtreeTask(items[i], origin, overlaps, rangeChanged))
	}
}
