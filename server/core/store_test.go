...
func (s *testStoreSuite) TestTopology(c *C) {
	stores := NewStoresInfo()
	labels := []string{"zone", "rack", "host"}
	allStores := []*StoreInfo{
		NewStoreInfoWithLabel(1, 1, map[string]string{"zone": "z1", "rack": "r1", "host": "h1"}),
		NewStoreInfoWithLabel(2, 1, map[string]string{"zone": "z1", "rack": "r1", "host": "h2"}),
		NewStoreInfoWithLabel(3, 1, map[string]string{"zone": "z1", "rack": "r2", "host": "h1"}),
		NewStoreInfoWithLabel(4, 1, map[string]string{"zone": "z2", "rack": "r1", "host": "h1"}),
		NewStoreInfoWithLabel(5, 1, map[string]string{"zone": "z2", "rack": "r2", "host": "h1"}),
		NewStoreInfoWithLabel(6, 1, map[string]string{"zone": "z3", "rack": "r1", "host": "h1"}),
	}
	for _, store := range allStores {
		stores.SetStore(store, labels)
	}
	zone1 := stores.topology["z1"]
	c.Assert(zone1, NotNil)
	c.Assert(stores.topology["z2"], NotNil)
	c.Assert(stores.topology["z3"], NotNil)

	racks := zone1.(map[string]interface{})
	rack1 := racks["r1"]
	c.Assert(rack1, NotNil)
	c.Assert(racks["r2"], NotNil)

	hosts := rack1.(map[string]interface{})
	c.Assert(hosts["h1"], NotNil)
	c.Assert(hosts["h2"], NotNil)

	c.Assert(stores.GetStoreTopoWeight(6, labels), Equals, 1.0/3.0)
	c.Assert(stores.GetStoreTopoWeight(4, labels), Equals, 1.0/3.0/2.0)
	c.Assert(stores.GetStoreTopoWeight(1, labels), Equals, 1.0/3.0/2.0/2.0)

	// update store label
	s6 := NewStoreInfoWithLabel(6, 1, map[string]string{"zone": "z2", "rack": "r2", "host": "h2"})
	stores.SetStore(s6, labels)

	c.Assert(stores.topology["z3"], IsNil)

	labels = []string{"rack", "host"}
	stores = NewStoresInfo()
	for _, store := range allStores {
		stores.SetStore(store, labels)
	}
	c.Assert(stores.GetStoreTopoWeight(6, labels), Equals, 0.25)
	c.Assert(stores.GetStoreTopoWeight(3, labels), Equals, 0.5)

	s7 := NewStoreInfoWithLabel(7, 1, map[string]string{"host": "h3"})
	stores.SetStore(s7, labels)
	c.Assert(stores.GetStoreTopoWeight(6, labels), Equals, 1.0/3.0/2.0)
	c.Assert(stores.GetStoreTopoWeight(3, labels), Equals, 1.0/3.0)
