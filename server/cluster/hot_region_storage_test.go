...
func (t *testHotRegionStorage) TestHotRegionWrite(c *C) {
	...
	for r, err := iter.Next(); r != nil && err == nil; r, err = iter.Next() {
		...
		c.Assert(reflect.DeepEqual(r.EndKey, regions[index].GetMeta().EndKey), IsTrue)
		index++
	}
	c.Assert(err, IsNil)
...
func (t *testHotRegionStorage) TestHotRegionDelete(c *C) {
	regionStorage, clear, err := newTestHotRegionStorage(10*time.Hour, 0)
	defer clear()
	c.Assert(err, IsNil)
	raft := regionStorage.cluster
	stats := statistics.StoreHotPeersStat{}
	...
	regions, statShows, start_time, _ :=
