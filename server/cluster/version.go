...
func MinSupportedVersion(v Feature) *semver.Version {
	...
	if !ok {
		log.Fatal("the corresponding version of the feature doesn't exist", zap.Int("feature-number", int(v)), zap.Error(errs.ErrInternalVersionFeatureNotExist.FastGenByArgs()))
	}
