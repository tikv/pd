...
// startGCTuner starts the GC tuner background job.
func (c *RaftCluster) startGCTuner() {
	...
	// Initialize GC tuner using the helper function
	state := gctuner.InitGCTuner(totalMem, c.getGCTunerConfig())
...
// getGCTunerConfig converts PD server config to gctuner.Config.
func (c *RaftCluster) getGCTunerConfig() *gctuner.Config {
