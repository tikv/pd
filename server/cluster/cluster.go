...
func (c *RaftCluster) InitCluster(
	...
	keyspaceGroupManager *keyspace.GroupManager) {
	c.BasicCluster, c.opt, c.storage, c.id = basicCluster, opt, storage, id
...
func (c *RaftCluster) Start(s Server) error {
	...
	c.regionStats = statistics.NewRegionStatistics(c.BasicCluster, c.opt, c.ruleManager, c.storeConfigManager)
...
func (c *RaftCluster) runStatsBackgroundJobs() {
	...
	for {
		...
		case <-c.ctx.Done():
			...
			return
		case <-ticker.C:
			c.hotStat.ObserveRegionsStats(c.GetStoresWriteRate())
		}
	}
}
...
func (c *RaftCluster) UpdateRegionsLabelLevelStats(regions []*core.RegionInfo) {
	for _, region := range regions {
		c.labelLevelStats.Observe(region, c.getStoresWithoutLabel(region, core.EngineKey, core.EngineTiFlash), c.opt.GetLocationLabels())
	}
}

func (c *RaftCluster) getStoresWithoutLabel(region *core.RegionInfo, key, value string) []*core.StoreInfo {
