...
func NewRaftCluster(ctx context.Context, clusterID uint64, basicCluster *core.BasicCluster, storage storage.Storage, regionSyncer *syncer.RegionSyncer, etcdClient *clientv3.Client,
	...
	return &RaftCluster{
		...
		core:                 basicCluster,
		storage:              storage,
		taskRunner:           ratelimit.NewAsyncRunner("heartbeat-async-task-runner", 1000000),
...
func (c *RaftCluster) processRegionHeartbeat(ctx context.Context, region *core.RegionInfo) error {
	...
	if !saveKV && !saveCache {
		...
		if hasRegionStats && c.regionStats.RegionStatsNeedUpdate(region) {
			runner.RunTask(
				...
				func(ctx context.Context) {
					// get region again to avoid the region is stale
					// concurrency cannot make sure the order anyway
					region = c.GetRegion(region.GetID())
	...
	if saveCache {
		...
		if overlaps, err = c.core.CheckAndPutSupTree(ctx, region); err != nil {
		...
		runner.RunTask(
			...
			func(ctx context.Context) {
				c.core.CheckAndPutSubTree(ctx, region)
			},
		)
