...
func Test(t *testing.T) {
	...
	for i := uint64(0); i < n; i++ {
		region := filter.SelectOneRegion(tc.RandLeaderRegions(i, []core.KeyRange{core.NewKeyRange("", "")}), []filter.RegionFilter{pendingFilter, downFilter})
		re.Equal(i, region.GetLeader().GetStoreId())

		region = filter.SelectOneRegion(tc.RandFollowerRegions(i, []core.KeyRange{core.NewKeyRange("", "")}), []filter.RegionFilter{pendingFilter, downFilter})
	...
	for i := uint64(0); i < n; i++ {
		...
		re.Nil(filter.SelectOneRegion(tc.RandLeaderRegions(i, []core.KeyRange{core.NewKeyRange("", "")}), []filter.RegionFilter{pendingFilter, downFilter}))
	}
	for i := uint64(0); i < n; i++ {
		re.Nil(filter.SelectOneRegion(tc.RandFollowerRegions(i, []core.KeyRange{core.NewKeyRange("", "")}), []filter.RegionFilter{pendingFilter, downFilter}))
	}
}
