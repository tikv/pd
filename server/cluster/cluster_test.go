...
func TestRegionHeartbeatHotStat(t *testing.T) {
	...
	err = cluster.processRegionHeartbeat(context.TODO(), region)
	re.NoError(err)
...
func TestBucketHeartbeat(t *testing.T) {
	...
	re.NoError(cluster.processRegionHeartbeat(context.TODO(), regions[0]))
	re.NoError(cluster.processRegionHeartbeat(context.TODO(), regions[1]))
	...
	re.NoError(cluster.processRegionHeartbeat(context.TODO(), newRegion))
	...
	re.NoError(cluster.processRegionHeartbeat(context.TODO(), newRegion2))
	re.Nil(cluster.GetRegion(uint64(1)).GetBuckets())
...
func TestRegionHeartbeat(t *testing.T) {
	...
	for i, region := range regions {
		...
		re.NoError(cluster.processRegionHeartbeat(context.TODO(), region))
		...
		re.Error(cluster.processRegionHeartbeat(context.TODO(), stale))
		checkRegions(re, cluster.core, regions[:i+1])
		checkRegionsKV(re, cluster.storage, regions[:i+1])
	...
	if storage := cluster.storage; storage != nil {
		...
		re.Error(cluster.processRegionHeartbeat(context.TODO(), overlapRegion))
		...
		tracer.Begin()
		ctx := context.TODO()
		ctx = context.WithValue(ctx, ctxutil.HeartbeatTracerKey, tracer)
...
func TestRegionFlowChanged(t *testing.T) {
	...
	processRegions := func(regions []*core.RegionInfo) {
		for _, r := range regions {
			cluster.processRegionHeartbeat(ctx, r)
		}
	}
...
func TestRegionSizeChanged(t *testing.T) {
	...
	cluster.processRegionHeartbeat(context.TODO(), region)
...
func TestConcurrentRegionHeartbeat(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		cluster.processRegionHeartbeat(context.TODO(), source)
	}()
	time.Sleep(100 * time.Millisecond)
	re.NoError(failpoint.Disable("github.com/tikv/pd/server/cluster/concurrentRegionHeartbeat"))
	re.NoError(cluster.processRegionHeartbeat(context.TODO(), target))
	wg.Wait()
...
func heartbeatRegions(re *require.Assertions, cluster *RaftCluster, regions []*core.RegionInfo) {
	...
	for _, r := range regions {
		re.NoError(cluster.processRegionHeartbeat(context.TODO(), r))
...
func TestHeartbeatSplit(t *testing.T) {
	...
	re.NoError(cluster.processRegionHeartbeat(context.TODO(), region1))
	...
	re.NoError(cluster.processRegionHeartbeat(context.TODO(), region2))
	...
	re.NoError(cluster.processRegionHeartbeat(context.TODO(), region3))
...
func TestUpdateStorePendingPeerCount(t *testing.T) {
	...
	re.NoError(tc.processRegionHeartbeat(context.TODO(), origin))
	time.Sleep(50 * time.Millisecond)
	...
	re.NoError(tc.processRegionHeartbeat(context.TODO(), newRegion))
	time.Sleep(50 * time.Millisecond)
...
func TestShouldRun(t *testing.T) {
	...
	for _, testCase := range testCases {
		...
		re.NoError(tc.processRegionHeartbeat(context.TODO(), nr))
	...
	re.Error(tc.processRegionHeartbeat(context.TODO(), newRegion))
