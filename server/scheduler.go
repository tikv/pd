...
func (s *grantLeaderScheduler) Schedule(cluster *clusterInfo) Operator {
	balancerCounter.WithLabelValues(s.GetName(), "schedule").Inc()
...
func (s *evictLeaderScheduler) Schedule(cluster *clusterInfo) Operator {
	...
	if region == nil {
		balancerCounter.WithLabelValues(s.GetName(), "no_leader").Inc()
		return nil
	}
...
func (s *shuffleRegionScheduler) Schedule(cluster *clusterInfo) Operator {
	...
	if region == nil {
		balancerCounter.WithLabelValues(s.GetName(), "no_region").Inc()
		return nil
	}
	...
	if newPeer == nil {
		balancerCounter.WithLabelValues(s.GetName(), "no_new_peer").Inc()
		return nil
	}
...
func scheduleRemovePeer(cluster *clusterInfo, schedulerName string, s Selector, filters ...Filter) (*RegionInfo, *metapb.Peer) {
	...
	if source == nil {
		balancerCounter.WithLabelValues(schedulerName, "no_store").Inc()
		return nil, nil
	}
	...
	if region == nil {
		balancerCounter.WithLabelValues(schedulerName, "no_region").Inc()
		return nil, nil
	}
...
func scheduleTransferLeader(cluster *clusterInfo, schedulerName string, s Selector, filters ...Filter) (*RegionInfo, *metapb.Peer) {
	...
	if mostLeaderDistance == 0 && leastLeaderDistance == 0 {
		balancerCounter.WithLabelValues(schedulerName, "already_balanced").Inc()
		return nil, nil
	}
	...
	if mostLeaderDistance > leastLeaderDistance {
		...
		if region == nil {
			balancerCounter.WithLabelValues(schedulerName, "no_leader_region").Inc()
			return nil, nil
		}
		targetStores := cluster.getFollowerStores(region)
		target := s.SelectTarget(targetStores)
		if target == nil {
			balancerCounter.WithLabelValues(schedulerName, "no_target_store").Inc()
			return nil, nil
		}
	...
	if region == nil {
		balancerCounter.WithLabelValues(schedulerName, "no_target_peer").Inc()
		return nil, nil
	}
