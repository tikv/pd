...
var statsTypes = []RegionStatisticType{
	MissPeer,
	ExtraPeer,
	DownPeer,
	PendingPeer,
...
func (r *RegionStatistics) Observe(region *core.RegionInfo, stores []*core.StoreInfo) {
	...
	if isRemoving {
		r.stats[OfflinePeer][regionID] = &RegionInfo{
			RegionInfo: region,
		}
...
func (r *RegionStatistics) Collect() {
	...
	regionStatusGauge.WithLabelValues("offline-region-count").Set(float64(len(r.stats[OfflinePeer])))
