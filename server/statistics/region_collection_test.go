...
func (t *testRegionStatisticsSuite) TestRegionStatistics(c *C) {
	...
	c.Assert(regionStats.stats[MergerRegion], HasLen, 1)
	...
	c.Assert(regionStats.offlineStats[MergerRegion], HasLen, 1)
	...
	c.Assert(regionStats.stats[MergerRegion], HasLen, 0)
	c.Assert(regionStats.stats[SplitRegion], HasLen, 1)
	c.Assert(regionStats.offlineStats[ExtraPeer], HasLen, 1)
	...
	c.Assert(regionStats.offlineStats[OfflinePeer], HasLen, 1)
	c.Assert(regionStats.offlineStats[MergerRegion], HasLen, 0)
	c.Assert(regionStats.offlineStats[SplitRegion], HasLen, 1)
