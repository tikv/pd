...
func (t *timestampOracle) SyncTimestamp(leadership *election.Leadership) error {
	tsoCounter.With(prometheus.Labels{"type": "sync", "dc": t.dcLocation}).Inc()
	...
	if err = t.saveTimestamp(leadership, save); err != nil {
		tsoCounter.With(prometheus.Labels{"type": "err_save_sync_ts", "dc": t.dcLocation}).Inc()
		return err
	}

	tsoCounter.With(prometheus.Labels{"type": "sync_ok", "dc": t.dcLocation}).Inc()
...
func (t *timestampOracle) resetUserTimestamp(leadership *election.Leadership, tso uint64, ignoreSmaller bool) error {
	...
	defer t.tsoMux.Unlock()
	if !leadership.Check() {
		tsoCounter.With(prometheus.Labels{"type": "err_lease_reset_ts", "dc": t.dcLocation}).Inc()
	...
	if typeutil.SubTimeByWallClock(nextPhysical, t.tsoMux.tso.physical) == 0 && int64(nextLogical) <= t.tsoMux.tso.logical {
		tsoCounter.With(prometheus.Labels{"type": "err_reset_small_counter", "dc": t.dcLocation}).Inc()
		if !ignoreSmaller {
	...
	if typeutil.SubTimeByWallClock(nextPhysical, t.tsoMux.tso.physical) < 0 {
		tsoCounter.With(prometheus.Labels{"type": "err_reset_small_ts", "dc": t.dcLocation}).Inc()
		if !ignoreSmaller {
	...
	if typeutil.SubTimeByWallClock(nextPhysical, t.tsoMux.tso.physical) >= t.maxResetTSGap() {
		tsoCounter.With(prometheus.Labels{"type": "err_reset_large_ts", "dc": t.dcLocation}).Inc()
	...
	if typeutil.SubTimeByWallClock(t.lastSavedTime.Load().(time.Time), nextPhysical) <= updateTimestampGuard {
		...
		if err = t.saveTimestamp(leadership, save); err != nil {
			tsoCounter.With(prometheus.Labels{"type": "err_save_reset_ts", "dc": t.dcLocation}).Inc()
			return err
		}
	}
	...
	tsoCounter.With(prometheus.Labels{"type": "reset_tso_ok", "dc": t.dcLocation}).Inc()
	return nil
}
...
func (t *timestampOracle) UpdateTimestamp(leadership *election.Leadership) error {
	...
	tsoGauge.With(prometheus.Labels{"type": "tso", "dc": t.dcLocation}).Set(float64(prevPhysical.Unix()))
	...
	tsoCounter.With(prometheus.Labels{"type": "save", "dc": t.dcLocation}).Inc()
	...
	if jetLag > 3*t.updatePhysicalInterval {
		...
		tsoCounter.With(prometheus.Labels{"type": "slow_save", "dc": t.dcLocation}).Inc()
	}

	if jetLag < 0 {
		tsoCounter.With(prometheus.Labels{"type": "system_time_slow", "dc": t.dcLocation}).Inc()
	}
	...
	if jetLag > updateTimestampGuard {
		...
		tsoCounter.With(prometheus.Labels{"type": "skip_save", "dc": t.dcLocation}).Inc()
		return nil
	}
	...
	if typeutil.SubTimeByWallClock(t.lastSavedTime.Load().(time.Time), next) <= updateTimestampGuard {
		save := next.Add(t.saveInterval)
		if err := t.saveTimestamp(leadership, save); err != nil {
			tsoCounter.With(prometheus.Labels{"type": "err_save_update_ts", "dc": t.dcLocation}).Inc()
			return err
		}
	}
	// save into memory
...
func (t *timestampOracle) getTS(leadership *election.Leadership, count uint32, suffixBits int) (pdpb.Timestamp, error) {
	...
	for i := 0; i < maxRetryCount; i++ {
		...
		if resp.GetLogical() >= maxLogical {
			...
			tsoCounter.With(prometheus.Labels{"type": "logical_overflow", "dc": t.dcLocation}).Inc()
