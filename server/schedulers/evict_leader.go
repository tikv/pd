...
func (s *evictLeaderScheduler) Schedule(cluster schedule.Cluster, dryRun bool) ([]*operator.Operator, []schedule.Plan) {
...
func scheduleEvictLeaderOnce(name, typ string, cluster schedule.Cluster, conf evictLeaderStoresConf) []*operator.Operator {
	...
	for _, storeID := range stores {
		...
		pendingFilter := filter.NewRegionPengdingFilter(name)
		downFilter := filter.NewRegionDownFilter(name)
		...
		filters = append(filters,
			&filter.LongTermStateFilter{ActionScope: name, TransferLeader: true},
			&filter.TemporaryStateFilter{ActionScope: name, TransferLeader: true})
