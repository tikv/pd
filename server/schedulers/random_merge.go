...
func (s *randomMergeScheduler) Schedule(cluster schedule.Cluster, dryRun bool) ([]*operator.Operator, []plan.Plan) {
	...
	store := filter.NewCandidates(cluster.GetStores()).
		FilterSource(cluster.GetOpts(), []filter.Filter{&filter.StoreStateFilter{ActionScope: s.conf.Name, MoveRegion: true}}).
		RandomPick()
	if store == nil {
	...
	region := filter.SelectOneRegion(cluster.RandLeaderRegions(store.GetID(), s.conf.Ranges), []filter.RegionFilter{pendingFilter, downFilter})
	if region == nil {
