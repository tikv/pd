...
func (s *testBalanceSuite) TestShouldBalance(c *C) {
	...
	for _, t := range tests {
		...
		solver := newSolver(kind, tc, oc.GetOpInfluence(tc))
		plan := newSchedulePlan()
		plan.source, plan.target, plan.region = tc.GetStore(1), tc.GetStore(2), tc.GetRegion(1)
		c.Assert(solver.shouldBalance("", plan), Equals, t.expectedResult)
	}
...
func (s *testBalanceSuite) TestTolerantRatio(c *C) {
	...
	for i, t := range tbl {
		...
		solver := newSolver(t.kind, tc, operator.OpInfluence{})
		plan := newSchedulePlan()
		plan.region = region
		c.Assert(solver.getTolerantResource(plan.region), Equals, t.expectTolerantResource(t.kind), Commentf("case #%d", i+1))
	}
}
