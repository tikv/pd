...
func (s *testBalanceLeaderSchedulerSuite) SetUpTest(c *C) {
	...
	lb, err := schedule.CreateScheduler(BalanceLeaderType, s.oc, storage.NewBuilder().WithMemoryBackend().Build(), schedule.ConfigSliceDecoder(BalanceLeaderType, []string{"", ""}))
	c.Assert(err, IsNil)
	s.lb = lb
}
...
func (s *testBalanceLeaderRangeSchedulerSuite) TestSingleRangeBalance(c *C) {
	...
	lb, err = schedule.CreateScheduler(BalanceLeaderType, s.oc, storage.NewBuilder().WithMemoryBackend().Build(), schedule.ConfigSliceDecoder(BalanceLeaderType, []string{"h", "n"}))
	c.Assert(err, IsNil)
	c.Assert(lb.Schedule(s.tc), IsNil)
	lb, err = schedule.CreateScheduler(BalanceLeaderType, s.oc, storage.NewBuilder().WithMemoryBackend().Build(), schedule.ConfigSliceDecoder(BalanceLeaderType, []string{"b", "f"}))
	c.Assert(err, IsNil)
	c.Assert(lb.Schedule(s.tc), IsNil)
	lb, err = schedule.CreateScheduler(BalanceLeaderType, s.oc, storage.NewBuilder().WithMemoryBackend().Build(), schedule.ConfigSliceDecoder(BalanceLeaderType, []string{"", "a"}))
	c.Assert(err, IsNil)
	c.Assert(lb.Schedule(s.tc), IsNil)
	lb, err = schedule.CreateScheduler(BalanceLeaderType, s.oc, storage.NewBuilder().WithMemoryBackend().Build(), schedule.ConfigSliceDecoder(BalanceLeaderType, []string{"g", ""}))
	c.Assert(err, IsNil)
	c.Assert(lb.Schedule(s.tc), IsNil)
	lb, err = schedule.CreateScheduler(BalanceLeaderType, s.oc, storage.NewBuilder().WithMemoryBackend().Build(), schedule.ConfigSliceDecoder(BalanceLeaderType, []string{"", "f"}))
	c.Assert(err, IsNil)
	c.Assert(lb.Schedule(s.tc), IsNil)
	lb, err = schedule.CreateScheduler(BalanceLeaderType, s.oc, storage.NewBuilder().WithMemoryBackend().Build(), schedule.ConfigSliceDecoder(BalanceLeaderType, []string{"b", ""}))
	c.Assert(err, IsNil)
	c.Assert(lb.Schedule(s.tc), IsNil)
}
...
func (s *testBalanceLeaderRangeSchedulerSuite) TestMultiRangeBalance(c *C) {
	...
	s.tc.UpdateStoreLeaderWeight(4, 2)
	s.tc.AddLeaderRegionWithRange(1, "a", "g", 1, 2, 3, 4)
	lb, err := schedule.CreateScheduler(BalanceLeaderType, s.oc, storage.NewBuilder().WithMemoryBackend().Build(), schedule.ConfigSliceDecoder(BalanceLeaderType, []string{"", "g", "o", "t"}))
	c.Assert(err, IsNil)
...
func (s *testBalanceRegionSchedulerSuite) TestBalance(c *C) {
	...
	sb, err := schedule.CreateScheduler(BalanceRegionType, oc, storage.NewBuilder().WithMemoryBackend().Build(), schedule.ConfigSliceDecoder(BalanceRegionType, []string{"", ""}))
	c.Assert(err, IsNil)
...
func (s *testRandomMergeSchedulerSuite) TestMerge(c *C) {
	...
	mb, err := schedule.CreateScheduler(RandomMergeType, oc, storage.NewBuilder().WithMemoryBackend().Build(), schedule.ConfigSliceDecoder(RandomMergeType, []string{"", ""}))
	c.Assert(err, IsNil)
...
func (s *testScatterRangeSuite) TestBalance(c *C) {
	...
	hb, err := schedule.CreateScheduler(ScatterRangeType, oc, storage.NewBuilder().WithMemoryBackend().Build(), schedule.ConfigSliceDecoder(ScatterRangeType, []string{"s_00", "s_50", "t"}))
	c.Assert(err, IsNil)
...
func (s *testScatterRangeSuite) TestBalanceWhenRegionNotHeartbeat(c *C) {
	...
	oc := schedule.NewOperatorController(s.ctx, nil, nil)
	hb, err := schedule.CreateScheduler(ScatterRangeType, oc, storage.NewBuilder().WithMemoryBackend().Build(), schedule.ConfigSliceDecoder(ScatterRangeType, []string{"s_00", "s_09", "t"}))
	c.Assert(err, IsNil)
