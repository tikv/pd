...
func (l *balanceAdjacentRegionScheduler) dispersePeer(cluster schedule.Cluster, region *core.RegionInfo) *schedule.Operator {
	...
	op := schedule.CreateMovePeerOperator("balance-adjacent-peer", region, schedule.OpAdjacent,	 leaderStoreID, newPeer.GetStoreId(), newPeer.GetId(), cluster.IsEnableRaftLearner())
	op.SetPriorityLevel(core.LowPriority)
