...
func (s *balanceRegionScheduler) transferPeer(cluster schedule.Cluster, region *core.RegionInfo, oldPeer *metapb.Peer) *operator.Operator {
	...
	if !shouldBalance(cluster, source, target, region, core.RegionKind, opInfluence) {
		EnableLeaderScheduleByCount := false
		log.Debug("skip balance region",
			...
			zap.Int64("source-influence", opInfluence.GetStoreInfluence(sourceID).ResourceSize(core.RegionKind, EnableLeaderScheduleByCount)),
			...
			zap.Int64("target-influence", opInfluence.GetStoreInfluence(targetID).ResourceSize(core.RegionKind, EnableLeaderScheduleByCount)),
