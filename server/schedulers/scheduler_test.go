...
func (s *testBalanceAdjacentRegionSuite) TestBalance(c *C) {
	...
	// to a new store
	checkTransferPeerWithLeaderTransfer(c, sc.Schedule(tc, schedule.NewOpInfluence(nil, tc))[0], 1, 4)
	...
	CheckTransferLeader(c, sc.Schedule(tc, schedule.NewOpInfluence(nil, tc))[0], 1, 2)
	tc.addLeaderRegionWithRange(2, "a", "b", 2, 1, 3)

	// transfer leader from store 1 to store 2 for region 3
	CheckTransferLeader(c, sc.Schedule(tc, schedule.NewOpInfluence(nil, tc))[0], 1, 4)
	...
	c.Assert(sc.Schedule(tc, schedule.NewOpInfluence(nil, tc)), IsNil)
	c.Assert(sc.Schedule(tc, schedule.NewOpInfluence(nil, tc)), IsNil)
	CheckTransferLeader(c, sc.Schedule(tc, schedule.NewOpInfluence(nil, tc))[0], 2, 4)
...
func (s *testRejectLeaderSuite) TestRejectLeader(c *C) {
	...
	c.Assert(err, IsNil)
	op := sl.Schedule(tc, schedule.NewOpInfluence(nil, tc))
	CheckTransferLeader(c, op[0], 1, 2)
