...
func (s *testBalanceAdjacentRegionSuite) TestBalance(c *C) {
	...
	c.Assert(err, IsNil)
	c.Assert(sc.Schedule(tc, schedule.NewDiffMap(nil, tc)), IsNil)
	...
	// to a new store
	checkTransferPeerWithLeaderTransfer(c, sc.Schedule(tc, schedule.NewDiffMap(nil, tc)), 1, 4)
	...
	CheckTransferLeader(c, sc.Schedule(tc, schedule.NewDiffMap(nil, tc)), 1, 2)
	tc.addLeaderRegionWithRange(2, "a", "b", 2, 1, 3)

	// transfer leader from store 1 to store 2 for region 3
	CheckTransferLeader(c, sc.Schedule(tc, schedule.NewDiffMap(nil, tc)), 1, 4)
	...
	c.Assert(sc.Schedule(tc, schedule.NewDiffMap(nil, tc)), IsNil)
	c.Assert(sc.Schedule(tc, schedule.NewDiffMap(nil, tc)), IsNil)
	CheckTransferLeader(c, sc.Schedule(tc, schedule.NewDiffMap(nil, tc)), 2, 4)
