...
func (s *testShuffleLeaderSuite) TestShuffle(c *C) {
	...
	sl, err := schedule.CreateScheduler(ShuffleLeaderType, schedule.NewOperatorController(ctx, nil, nil), storage.NewMemoryStorage(), schedule.ConfigSliceDecoder(ShuffleLeaderType, []string{"", ""}))
	c.Assert(err, IsNil)
...
func (s *testRejectLeaderSuite) TestRejectLeader(c *C) {
	...
	sl, err := schedule.CreateScheduler(LabelType, oc, storage.NewMemoryStorage(), schedule.ConfigSliceDecoder(LabelType, []string{"", ""}))
	c.Assert(err, IsNil)
	op := sl.Schedule(tc)
	...
	// any operators.
	bs, err := schedule.CreateScheduler(BalanceLeaderType, oc, storage.NewMemoryStorage(), schedule.ConfigSliceDecoder(BalanceLeaderType, []string{"", ""}))
	c.Assert(err, IsNil)
	op = bs.Schedule(tc)
	c.Assert(op, IsNil)

	// Can't evict leader from store2, neither.
	el, err := schedule.CreateScheduler(EvictLeaderType, oc, storage.NewMemoryStorage(), schedule.ConfigSliceDecoder(EvictLeaderType, []string{"2"}))
	c.Assert(err, IsNil)
	op = el.Schedule(tc)
	c.Assert(op, IsNil)
...
func (s *testRejectLeaderSuite) TestRemoveRejectLeader(c *C) {
	...
	el, err := schedule.CreateScheduler(EvictLeaderType, oc, storage.NewMemoryStorage(), schedule.ConfigSliceDecoder(EvictLeaderType, []string{"1"}))
	c.Assert(err, IsNil)
...
func (s *testShuffleHotRegionSchedulerSuite) TestBalance(c *C) {
	...
	hb, err := schedule.CreateScheduler(ShuffleHotRegionType, schedule.NewOperatorController(ctx, nil, nil), storage.NewMemoryStorage(), schedule.ConfigSliceDecoder("shuffle-hot-region", []string{"", ""}))
	c.Assert(err, IsNil)
...
func (s *testHotRegionSchedulerSuite) TestAbnormalReplica(c *C) {
	...
	hb, err := schedule.CreateScheduler(statistics.Read.String(), schedule.NewOperatorController(ctx, nil, nil), storage.NewMemoryStorage(), nil)
	c.Assert(err, IsNil)
...
func (s *testEvictLeaderSuite) TestEvictLeader(c *C) {
	...
	sl, err := schedule.CreateScheduler(EvictLeaderType, schedule.NewOperatorController(ctx, nil, nil), storage.NewMemoryStorage(), schedule.ConfigSliceDecoder(EvictLeaderType, []string{"1"}))
	c.Assert(err, IsNil)
...
func (s *testShuffleRegionSuite) TestShuffle(c *C) {
	...
	sl, err := schedule.CreateScheduler(ShuffleRegionType, schedule.NewOperatorController(ctx, nil, nil), storage.NewMemoryStorage(), schedule.ConfigSliceDecoder(ShuffleRegionType, []string{"", ""}))
	c.Assert(err, IsNil)
	c.Assert(sl.IsScheduleAllowed(tc), IsTrue)
	c.Assert(sl.Schedule(tc), IsNil)
