...
func (s *testHotSchedulerSuite) TestGCPendingOpInfos(c *C) {
	...
	sche, err := schedule.CreateScheduler(HotRegionType, schedule.NewOperatorController(ctx, tc, nil), core.NewMemoryStorage(), schedule.ConfigJSONDecoder([]byte("null")))
	c.Assert(err, IsNil)
...
func (s *testHotWriteRegionSchedulerSuite) TestByteRateOnly(c *C) {
	...
	hb, err := schedule.CreateScheduler(HotWriteRegionType, schedule.NewOperatorController(ctx, nil, nil), core.NewMemoryStorage(), nil)
	c.Assert(err, IsNil)
...
func (s *testHotWriteRegionSchedulerSuite) TestByteRateOnlyWithTiFlash(c *C) {
	...
	sche, err := schedule.CreateScheduler(HotWriteRegionType, schedule.NewOperatorController(ctx, nil, nil), core.NewMemoryStorage(), nil)
	c.Assert(err, IsNil)
	hb := sche.(*hotScheduler)
...
func (s *testHotReadRegionSchedulerSuite) TestHotReadPeerSchedule(c *C) {
	...
	sche, err := schedule.CreateScheduler(HotReadRegionType, schedule.NewOperatorController(ctx, tc, nil), core.NewMemoryStorage(), schedule.ConfigJSONDecoder([]byte("null")))
	c.Assert(err, IsNil)
	hb := sche.(*hotScheduler)
...
func (s *testHotSchedulerSuite) TestCompatibilityConfig(c *C) {
	...
	hb, err := schedule.CreateScheduler(HotRegionType, schedule.NewOperatorController(ctx, tc, nil), core.NewMemoryStorage(), schedule.ConfigSliceDecoder("hot-region", nil))
	c.Assert(err, IsNil)
	checkPriority(c, hb.(*hotScheduler), tc, [3][2]int{
		{statistics.QueryDim, statistics.ByteDim},
		{statistics.KeyDim, statistics.ByteDim},
	...
	hb, err = schedule.CreateScheduler(HotRegionType, schedule.NewOperatorController(ctx, tc, nil), core.NewMemoryStorage(),
	...
	c.Assert(err, IsNil)
	hb, err = schedule.CreateScheduler(HotRegionType, schedule.NewOperatorController(ctx, tc, nil), core.NewMemoryStorage(), schedule.ConfigJSONDecoder(data))
	c.Assert(err, IsNil)
	checkPriority(c, hb.(*hotScheduler), tc, [3][2]int{
		{statistics.ByteDim, statistics.KeyDim},
		{statistics.KeyDim, statistics.ByteDim},
	...
	err = storage.SaveScheduleConfig(HotRegionName, data)
	c.Assert(err, IsNil)
	hb, err = schedule.CreateScheduler(HotRegionType, schedule.NewOperatorController(ctx, nil, nil), core.NewMemoryStorage(), schedule.ConfigJSONDecoder(data))
	c.Assert(err, IsNil)
	checkPriority(c, hb.(*hotScheduler), tc, [3][2]int{
