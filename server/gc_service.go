...
const (
	gcStateWatchReceiveBatchSizeHint = 1024
...
func (*GrpcServer) consumeGCStateWatch(
	...
	for {
		// Note that this is only a hint for the batch size when sending responses, which means, it only tells when we
		// stop receiving, but not a strong limit to the batch size. GCStateManager may batch the results when sending
		// them out, and it won't be split here, so the actual batch size in the responses may be larger than the hint.
		batchSizeHint := gcStateWatchReceiveBatchSizeHint
		failpoint.Inject("gcStateWatchReceiveBatchSize", func(val failpoint.Value) {
			batchSizeHint = val.(int)
		})

		gcStates, err := watcher.RecvBatch(batchSizeHint)
		if err != nil {
