...
func checkAddPeer(c *C, op *schedule.Operator, storeID uint64) {
	...
	c.Assert(op.Step(0), Equals, schedule.AddPeer{ToStore: storeID})
}

func checkRemovePeer(c *C, op *schedule.Operator, storeID uint64) {
	if op.Len() == 1 {
		c.Assert(op.Step(0), Equals, schedule.RemovePeer{FromStore: storeID})
	} else {
		c.Assert(op.Len(), Equals, 2)
		c.Assert(op.Step(0).(schedule.TransferLeader).FromStore, Equals, storeID)
		c.Assert(op.Step(1), Equals, schedule.RemovePeer{FromStore: storeID})
	}
}

func checkTransferPeer(c *C, op *schedule.Operator, sourceID, targetID uint64) {
	if op.Len() == 2 {
		c.Assert(op.Step(0), Equals, schedule.AddPeer{ToStore: targetID})
		c.Assert(op.Step(1), Equals, schedule.RemovePeer{FromStore: sourceID})
	} else {
		c.Assert(op.Len(), Equals, 3)
		c.Assert(op.Step(0), Equals, schedule.AddPeer{ToStore: targetID})
		c.Assert(op.Step(1).(schedule.TransferLeader).FromStore, Equals, sourceID)
		c.Assert(op.Step(2), Equals, schedule.RemovePeer{FromStore: sourceID})
	}
}
