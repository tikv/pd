...
const defaultTestLeaderLeaseTimeout = 1
...
func (s *testLeadershipSuite) TestLeadership(c *C) {
	...
	err = leadership1.Campaign(defaultTestLeaderLeaseTimeout, "test_leader_1")
	c.Assert(err, IsNil)
	// leadership2 starts then and can not get the leadership
	err = leadership2.Campaign(defaultTestLeaderLeaseTimeout, "test_leader_2")
	c.Assert(err, NotNil)
	...
	// Sleep longer than the defaultTestLeaderLeaseTimeout to wait for the lease expires
	...
	// Sleep longer than the defaultTestLeaderLeaseTimeout
	time.Sleep((defaultTestLeaderLeaseTimeout + 1) * time.Second)
