...
func (s *testRegionsInfoSuite) Test(c *C) {
	...
	for i := uint64(0); i < n; i++ {
		region := cluster.RandLeaderRegion(i, opt.HealthRegion(cluster))
		c.Assert(region.GetLeader().GetStoreId(), Equals, i)

		region = cluster.RandFollowerRegion(i, opt.HealthRegion(cluster))
	...
	for i := uint64(0); i < n; i++ {
		...
		c.Assert(cluster.RandLeaderRegion(i, opt.HealthRegion(cluster)), IsNil)
	}
	for i := uint64(0); i < n; i++ {
		c.Assert(cluster.RandFollowerRegion(i, opt.HealthRegion(cluster)), IsNil)
	}
}
