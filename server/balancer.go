...
func (l *balanceLeaderScheduler) Schedule(cluster *clusterInfo) Operator {
	balancerCounter.WithLabelValues(l.GetName(), "schedule").Inc()
	...
	if !shouldBalance(source, target, l.GetResourceKind()) {
		balancerCounter.WithLabelValues(l.GetName(), "skip").Inc()
		return nil
	}
	l.limit = adjustBalanceLimit(cluster, l.GetResourceKind())
	balancerCounter.WithLabelValues(l.GetName(), "new_opeartor").Inc()
...
func (s *balanceRegionScheduler) Schedule(cluster *clusterInfo) Operator {
	...
	if len(region.GetPeers()) != s.rep.GetMaxReplicas() {
		balancerCounter.WithLabelValues(s.GetName(), "abnormal_replica").Inc()
		return nil
	}
...
func (s *balanceRegionScheduler) transferPeer(cluster *clusterInfo, region *RegionInfo, oldPeer *metapb.Peer) Operator {
	...
	if newPeer == nil {
		balancerCounter.WithLabelValues(s.GetName(), "no_peer").Inc()
		return nil
	}

	target := cluster.getStore(newPeer.GetStoreId())
	if !shouldBalance(source, target, s.GetResourceKind()) {
		balancerCounter.WithLabelValues(s.GetName(), "skip").Inc()
		return nil
	}
...
func (h *balanceHotRegionScheduler) Schedule(cluster *clusterInfo) Operator {
	balancerCounter.WithLabelValues(h.GetName(), "schedule").Inc()
	h.calcScore(cluster)

	// balance by peer
	srcRegion, srcPeer, destPeer := h.balanceByPeer(cluster)
	if srcRegion != nil {
		balancerCounter.WithLabelValues(h.GetName(), "move_peer").Inc()
	...
	if srcRegion != nil {
		balancerCounter.WithLabelValues(h.GetName(), "move_leader").Inc()
		return newPriorityTransferLeader(srcRegion, newLeader)
	}

	balancerCounter.WithLabelValues(h.GetName(), "skip").Inc()
	return nil
}
