...
type OperatorStep interface {
	...
	Influence(stores *core.StoresInfo, region *core.RegionInfo)
}
...
func (tl TransferLeader) Influence(stores *core.StoresInfo, region *core.RegionInfo) {
	from := stores.GetStore(tl.FromStore)
	to := stores.GetStore(tl.ToStore)
	from.OpInfluence.LeaderSize -= int(region.ApproximateSize)
	from.OpInfluence.LeaderCount--
	to.OpInfluence.LeaderSize += int(region.ApproximateSize)
	to.OpInfluence.LeaderCount++
	stores.SetStore(from)
	stores.SetStore(to)
}
...
// Influence calculates the store difference that current step make
func (ap AddPeer) Influence(stores *core.StoresInfo, region *core.RegionInfo) {
	to := stores.GetStore(ap.ToStore)
	to.OpInfluence.RegionSize += int(region.ApproximateSize)
	to.OpInfluence.RegionCount++
	stores.SetStore(to)
}
...
// Influence calculates the store difference that current step make
func (rp RemovePeer) Influence(stores *core.StoresInfo, region *core.RegionInfo) {
	from := stores.GetStore(rp.FromStore)
	from.OpInfluence.RegionSize -= int(region.ApproximateSize)
	from.OpInfluence.RegionCount--
	stores.SetStore(from)
}
...
func (o *Operator) Influence(stores *core.StoresInfo, region *core.RegionInfo) {
	if o.IsTimeout() {
		return
	}
	...
	for step := atomic.LoadInt32(&o.currentStep); int(step) < len(o.steps); step++ {
		if !o.steps[int(step)].IsFinish(region) {
			o.steps[int(step)].Influence(stores, region)
		}
	}
}
