...
func (ap AddPeer) String() string {
	return fmt.Sprintf("add peer on store %v", ap.ToStore)
}
...
func CreateMovePeerOperator(desc string, region *core.RegionInfo, kind core.ResourceKind, oldStore, newStore uint64) *Operator {
	...
	return NewOperator(desc, region.GetId(), kind, AddPeer{ToStore: newStore}, RemovePeer{FromStore: oldStore})
}
