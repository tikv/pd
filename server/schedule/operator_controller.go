		...
		default:
			if oc.removeOperatorWithoutBury(op) {
				...
				log.Error("dispatching operator with unexpected status",
					...
					zap.Reflect("operator", op), zap.Error(errs.ErrInternalOperatorNotEnd.FastGenByArgs()))
	...
	for i := 0; i < len(ops); i++ {
		...
		if op.Kind()&operator.OpMerge != 0 {
			if i+1 >= len(ops) {
				...
				log.Error("orphan merge operators found", zap.String("desc", desc), zap.Error(errs.ErrInternalOperatorMerge.FastGenByArgs()))
				oc.Unlock()
				return added
			}
			if ops[i+1].Kind()&operator.OpMerge == 0 {
				log.Error("merge operator should be paired", zap.String("desc",
					ops[i+1].Desc()), zap.Error(errs.ErrInternalOperatorMerge.FastGenByArgs()))
				oc.Unlock()
				return added
			}
			isMerge = true
	...
	for _, op := range ops {
		...
		if op.Status() != operator.CREATED {
			log.Error("trying to add operator with unexpected status",
				zap.Uint64("region-id", op.RegionID()),
				zap.String("status", operator.OpStatusToString(op.Status())),
				zap.Reflect("operator", op), zap.Error(errs.ErrInternalOperatorNotStart.FastGenByArgs()))
			failpoint.Inject("unexpectedOperator", func() {
				panic(op)
			})
			operatorWaitCounter.WithLabelValues(op.Desc(), "add_canceled").Inc()
...
func (oc *OperatorController) SendScheduleCommand(region *core.RegionInfo, step operator.OpStep, source string) {
	...
	case operator.SplitRegion:
		...
		oc.hbStreams.SendMsg(region, cmd)
	default:
		log.Error("unknown operator step", zap.Reflect("step", step), zap.Error(errs.ErrInternalOperatorStepUnknown.FastGenByArgs()))
	}
}
