...
func (suite *createOperatorTestSuite) TestCreateSplitRegionOperator() {
	...
	for _, testCase := range testCases {
		...
		for i := 0; i < op.Len(); i++ {
			...
			default:
				log.Error("TestCreateSplitRegionOperator meets error ", zap.String("unexpected type", step.String()))
			}
		}
	}
}
...
func (suite *createOperatorTestSuite) TestCreateTransferLeaderOperator() {
	...
	for _, testCase := range testCases {
		...
		default:
			log.Error("TestCreateTransferLeaderOperator meets error ", zap.String("unexpected type", step.String()))
		}
	}
}
...
func (suite *createOperatorTestSuite) TestCreateLeaveJointStateOperator() {
	...
	for _, testCase := range testCases {
		...
		for i := 0; i < op.Len(); i++ {
			...
			default:
				log.Error("TestCreateLeaveJointStateOperator meets error ", zap.String("unexpected type", step.String()))
			}
		}
...
func (suite *createOperatorTestSuite) TestCreateMoveRegionOperator() {
	...
	for _, testCase := range testCases {
		...
		if testCase.expectedError == nil {
			...
			suite.Error(err)
			suite.True(strings.Contains(err.Error(), testCase.expectedError.Error()))
			continue
		}
		suite.NotNil(op)

		suite.Equal(len(testCase.steps), op.Len())
		...
		for i := 0; i < op.Len(); i++ {
			...
			default:
				log.Error("TestCreateMoveRegionOperator meets error ", zap.String("unexpected type", step.String()))
			}
		}
	}
}
	...
	for _, testCase := range testCases {
		...
		for i := 0; i < op.Len(); i++ {
			...
			case RemovePeer:
				suite.Equal(testCase.steps[i].(RemovePeer).FromStore, step.FromStore)
			default:
				log.Error("TestMoveRegionWithoutJointConsensus meets error ", zap.String("unexpected type", step.String()))
			}
		}
	}
}
