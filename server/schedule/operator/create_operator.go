...
// CreateSplitRegionOperator creates an operator that splits a region by loads.
func CreateSplitRegionOperator(desc string, region *core.RegionInfo, kind OpKind, policy pdpb.CheckPolicy, keys [][]byte, opts []float64) *Operator {
	step := SplitRegion{
		...
		SplitKeys:   keys,
		Opts:        opts,
		IsHotRegion: kind&OpHotRegion != 0,
	}
	...
	if len(opts) == 3 {
		brief += fmt.Sprintf(", with options: split into %f parts under %d-th dimension, rw-type: %d, is-hot-region: %v", opts[1], uint64(opts[0]), uint64(opts[2]), step.IsHotRegion)
	}
