...
func (s *testScatterRegionSuite) scatter(c *C, numStores, numRegions uint64, useRules bool) {
	...
	for i := uint64(1); i <= numRegions; i++ {
		...
		if op, _ := scatterer.Scatter(region, "", operator.OpDefault); op != nil {
...
func (s *testScatterRegionSuite) TestScatterCheck(c *C) {
	...
	for _, testcase := range testcases {
		...
		scatterer := NewRegionScatterer(ctx, tc)
		_, err := scatterer.Scatter(testcase.checkRegion, "", operator.OpDefault)
		if testcase.needFix {
			c.Assert(err, NotNil)
		...
		for i := 0; i < 100; i++ {
			for j := 0; j < testcase.groupCount; j++ {
				scatterer.scatterRegion(tc.AddLeaderRegion(uint64(regionID), 1, 2, 3),
					fmt.Sprintf("group-%v", j), operator.OpDefault)
				regionID++
			}
		}
...
func (s *testScatterRegionSuite) TestScattersGroup(c *C) {
	...
	for _, testcase := range testcases {
		...
		scatterer.ScatterRegions(regions, failures, group, 3, operator.OpDefault)
		max := uint64(0)
		min := uint64(math.MaxUint64)
...
func (s *testScatterRegionSuite) TestRegionFromDifferentGroups(c *C) {
	...
	for i := 1; i <= regionCount; i++ {
		...
		scatterer.scatterRegion(tc.AddLeaderRegion(uint64(i), uint64(p[0])+1, uint64(p[1])+1, uint64(p[2])+1), fmt.Sprintf("t%d", i), operator.OpDefault)
	}
...
func (s *testScatterRegionSuite) TestSelectedStores(c *C) {
	...
	for i := uint64(1); i < 20; i++ {
		...
		op := scatterer.scatterRegion(region, group, operator.OpDefault)
