...
func (c *clusterInfo) updateStoreStatus(id uint64) {
	...
	c.Stores.SetPendingLearnerCount(id, c.Regions.GetStorePendingLearnerCount(id))
...
func (c *clusterInfo) handleRegionHeartbeat(region *core.RegionInfo) error {
	...
	if origin == nil {
		...
		if len(region.DownPeers) > 0 || len(region.PendingPeers) > 0 || len(region.PendingLearners) > 0 || len(region.DownLearners) > 0 {
			saveCache = true
		}
		if len(origin.DownPeers) > 0 || len(origin.PendingPeers) > 0 || len(origin.PendingLearners) > 0 || len(origin.DownLearners) > 0 {
			saveCache = true
		}
