...
func (s *Server) startServer(ctx context.Context) error {
	...
	s.tsoAllocatorManager = tso.NewAllocatorManager(
		s.member, s.storage, s.rootPath, s.cfg.IsLocalTSOEnabled(), s.cfg.GetTSOSaveInterval(), s.cfg.GetTSOUpdatePhysicalInterval(), s.cfg.GetTLSConfig(),
		func() time.Duration { return s.persistOptions.GetMaxResetTSGap() })
	// Set up the Global TSO Allocator here, it will be initialized once the PD campaigns leader successfully.
	s.tsoAllocatorManager.SetUpAllocator(ctx, s.storage, tso.GlobalDCLocation, s.member.GetLeadership())
