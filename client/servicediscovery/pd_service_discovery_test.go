...
func (suite *serviceClientTestSuite) SetupSuite() {
	...
	for range 10 {
		leaderConn, err1 := grpc.Dial(suite.leaderServer.addr, grpc.WithTransportCredentials(insecure.NewCredentials()))     //nolint:staticcheck // grpc.Dial remains supported throughout grpc 1.x.
		followerConn, err2 := grpc.Dial(suite.followerServer.addr, grpc.WithTransportCredentials(insecure.NewCredentials())) //nolint:staticcheck // grpc.Dial remains supported throughout grpc 1.x.
...
func TestGRPCDialOption(t *testing.T) {
	...
	cli.option.GRPCDialOptions = []grpc.DialOption{grpc.WithBlock()} //nolint:staticcheck // grpc.WithBlock remains supported throughout grpc 1.x.
