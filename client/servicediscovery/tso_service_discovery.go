...
func (c *tsoServiceDiscovery) updateMember() error {
	...
	// Maybe the url is not one of the group members, so it can't get the primary URL but can get the keyspace meta info.
	// If met noPrimaryUrlErr error, it should try to get primary url from the member url.
