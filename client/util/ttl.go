...
// Remove remove key
func (c *TTLString) Remove(key uint64) {
...
func NewTTLKey(ctx context.Context, gcInterval, ttl time.Duration) *TTLKey {
	return &TTLKey{
		ttlCache: newTTL(ctx, gcInterval, ttl),
	}
}
...
func (c *TTLKey) Get(key interface{}) (interface{}, bool) {
	return c.ttlCache.get(key)
}

// Put saves an ID in cache.
func (c *TTLKey) Put(key interface{}) {
	c.ttlCache.put(key, nil)
}

// GetAllKeys returns all keys.
func (c *TTLKey) GetAllKeys() []interface{} {
	return c.ttlCache.getKeys()
}

// Exists checks if a key exists in cache.
func (c *TTLKey) Exists(key interface{}) bool {
	_, ok := c.ttlCache.get(key)
	return ok
}

// Remove remove key
func (c *TTLKey) Remove(key uint64) {
	c.ttlCache.remove(key)
}

// PutWithTTL puts an item into cache with specified TTL.
func (c *TTLKey) PutWithTTL(key interface{}, ttl time.Duration) {
	c.ttlCache.putWithTTL(key, nil, ttl)
}
