...
func (c *client) GetResourceGroup(ctx context.Context, resourceGroupName string) (*rmpb.ResourceGroup, error) {
	...
	if err != nil {
		c.gRPCErrorHandler(err)
		return nil, errs.ErrClientGetResourceGroup.FastGenByArgs(resourceGroupName, err.Error())
	}
	resErr := resp.GetError()
	if resErr != nil {
		return nil, errs.ErrClientGetResourceGroup.FastGenByArgs(resourceGroupName, resErr.Message)
	}
