...
func (s *testErrorSuite) TestError(c *C) {
	...
	rfc := `[error="[PD:url:ErrURLParse]parse url error"]`
	log.Error("test", zap.Error(ErrURLParse.FastGenByArgs()))
	c.Assert(strings.Contains(lg.Message(), rfc), IsTrue)
	err := errors.New("test error")
	log.Error("test", ZapError(ErrURLParse, err))
	rfc = `[error="[PD:url:ErrURLParse]test error"]`
	c.Assert(strings.Contains(lg.Message(), rfc), IsTrue)
}

func (s *testErrorSuite) TestErrorEqual(c *C) {
	err1 := ErrSecurityConfig.FastGenByArgs()
	err2 := ErrSecurityConfig.FastGenByArgs()
	c.Assert(errors.ErrorEqual(err1, err2), IsTrue)

	err := errors.New("test")
	err1 = ErrSecurityConfig.Wrap(err).FastGenWithCause()
	err2 = ErrSecurityConfig.Wrap(err).FastGenWithCause()
	c.Assert(errors.ErrorEqual(err1, err2), IsTrue)
	...
	err1 = ErrSecurityConfig.Wrap(err3).FastGenWithCause()
	err2 = ErrSecurityConfig.Wrap(err4).FastGenWithCause()
	c.Assert(errors.ErrorEqual(err1, err2), IsTrue)
...
func (s *testErrorSuite) TestErrorWithStack(c *C) {
	...
	log.Error("test", ZapError(ErrSecurityConfig.Wrap(err).GenWithStackByCause()))
	m1 := lg.Message()
