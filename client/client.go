	...
	for {
		...
		if stream == nil {
			...
			if err != nil {
				...
				log.Error("[pd] create tso stream error", zap.Error(errs.ErrGRPCTso.FastGenByArgs()), zap.NamedError("cause", err))
		...
		if err != nil {
			...
			log.Error("[pd] getTS error", zap.Error(errs.ErrGRPCTso.FastGenByArgs()), zap.NamedError("cause", err))
			c.ScheduleCheckLeader()
			cancel()
...
func (c *client) Close() {
	...
	for _, cc := range c.connMu.clientConns {
		if err := cc.Close(); err != nil {
			log.Error("[pd] failed close grpc clientConn", zap.Error(errs.ErrGRPCClose.FastGenByArgs()), zap.NamedError("cause", err))
		}
	}
}
