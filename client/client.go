...
type Client interface {
	...
	// GetTSAsync gets a timestamp from PD, without block the caller and provided a tsoRequest pool.
	GetTSAsyncWithPool(ctx context.Context, pool *sync.Pool) TSFuture
...
// TsoRequest presents the request to get tso.
...
func (c *client) GetTSAsync(ctx context.Context) TSFuture {
	return c.GetTSAsyncWithPool(ctx, &tsoReqPool)
}

func (c *client) GetTSAsyncWithPool(ctx context.Context, pool *sync.Pool) TSFuture {
	if span := opentracing.SpanFromContext(ctx); span != nil {
		span = opentracing.StartSpan("GetTSAsync", opentracing.ChildOf(span.Context()))
		ctx = opentracing.ContextWithSpan(ctx, span)
	}
	req := pool.Get().(*TsoRequest)
	req.start = time.Now()
	req.ctx = ctx
	req.physical = 0
	req.logical = 0
