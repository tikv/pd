...
func (c *client) GetLastTS(ctx context.Context) (physical int64, logical int64, err error) {
	lastTSOInterface, ok := c.lastTSMap.Load(globalDCLocation)
	if !ok {
		err := errors.New("func GetLastLocalTS fail : get last local TS before get first TS or something wrong happen")
		log.Error("[pd] get last local TS before get first TS or something wrong happen", zap.String("dc-location", globalDCLocation), errs.ZapError(err))
		return 0, 0, err
	}
	lastTSOPointer := lastTSOInterface.(*lastTSO)
