...
type tsoServiceProvider interface {
	...
	updateConnectionCtxs(ctx context.Context, dc string, connectionCtxs *cctx.Manager[*tsoStream]) bool
}
...
func newTSODispatcher(
	...
	td := &tsoDispatcher{
		...
		dc:             dc,
		provider:       provider,
		connectionCtxs: cctx.NewManager[*tsoStream](),
		tsoRequestCh:   tsoRequestCh,
		tsDeadlineCh:   make(chan *deadline, tokenChCapacity),
		batchBufferPool: &sync.Pool{
			New: func() any {
				return batch.NewController[*tsoRequest](
					maxBatchSize*2,
...
func (td *tsoDispatcher) handleDispatcher(wg *sync.WaitGroup) {
	var (
		...
		batchController *batch.Controller[*tsoRequest]
	)
	...
	defer func() {
		...
		connectionCtxs.ReleaseAll()
		if batchController != nil && batchController.GetCollectedRequestCount() != 0 {
...
tsoBatchLoop:
	for {
		...
		if batchController == nil {
			batchController = td.batchBufferPool.Get().(*batch.Controller[*tsoRequest])
		}
	...
	streamChoosingLoop:
		for {
			connectionCtx := connectionCtxs.RandomlyPick()
			...
			if stream == nil {
				...
				case <-streamLoopTimer.C:
					...
					log.Warn("[tso] create tso stream error", zap.String("dc-location", dc), errs.ZapError(err))
			...
			case <-streamCtx.Done():
				...
				connectionCtxs.Release(streamURL)
				stream = nil
				continue
			default:
			}
			...
			if err = stream.GetRecvError(); err != nil {
				exit := !td.handleProcessRequestError(ctx, bo, streamURL, err)
				stream = nil
				if exit {
					td.cancelCollectedRequests(batchController, invalidStreamID, errors.WithStack(ctx.Err()))
					return
...
func (td *tsoDispatcher) handleProcessRequestError(ctx context.Context, bo *retry.Backoffer, streamURL string, err error) bool {
	select {
	case <-ctx.Done():
		return false
	default:
	...
	td.connectionCtxs.Release(streamURL)
...
func (td *tsoDispatcher) processRequests(
	stream *tsoStream, dcLocation string, tbc *batch.Controller[*tsoRequest], done chan struct{},
) error {
	...
	var (
		count              = int64(tbc.GetCollectedRequestCount())
		svcDiscovery       = td.provider.getServiceDiscovery()
	...
	err := stream.processRequests(
		...
		dcLocation, count, tbc.GetExtraBatchingStartTime(), cb)
	if err != nil {
		close(done)
...
func tsoRequestFinisher(physical, firstLogical int64, suffixBits uint32, streamID string) batch.FinisherFunc[*tsoRequest] {
...
func (td *tsoDispatcher) cancelCollectedRequests(tbc *batch.Controller[*tsoRequest], streamID string, err error) {
	td.tokenCh <- struct{}{}
	tbc.FinishCollectedRequests(tsoRequestFinisher(0, 0, 0, streamID), err)
}

func (td *tsoDispatcher) doneCollectedRequests(tbc *batch.Controller[*tsoRequest], physical int64, firstLogical int64, suffixBits uint32, streamID string) {
	td.tokenCh <- struct{}{}
