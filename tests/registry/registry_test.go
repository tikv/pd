...
func TestRegistryService(t *testing.T) {
	...
	cc, err := grpc.DialContext(ctx, strings.TrimPrefix(leader.GetAddr(), "http://"), grpc.WithTransportCredentials(insecure.NewCredentials())) //nolint:staticcheck // grpc.DialContext remains supported throughout grpc 1.x.
	re.NoError(err)
	defer cc.Close()
