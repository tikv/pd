...
func (suite *regionTestSuite) checkRegionsReplicated(cluster *tests.TestCluster) {
	...
	err := testutil.CheckGetJSON(tests.TestDialClient, url, nil, testutil.Status(re, http.StatusBadRequest))
	re.NoError(err)
	...
	err = testutil.CheckGetJSON(tests.TestDialClient, url, nil, testutil.StatusOK(re))
	re.NoError(err)
	...
	testutil.Eventually(re, func() bool {
		if err := testutil.TryReadGetJSON(tests.TestDialClient, url, &status); err != nil {
			return false
		}
...
func (suite *regionTestSuite) checkRegion(cluster *tests.TestCluster) {
	...
	re.NoError(testutil.CheckGetJSON(tests.TestDialClient, url, nil, testutil.Status(re, http.StatusNotFound)))
	...
	re.NoError(testutil.ReadGetJSON(re, tests.TestDialClient, url, r2))
	r2.Adjust()
...
func (suite *regionTestSuite) checkRegionCheck(cluster *tests.TestCluster) {
	...
	testutil.Eventually(re, func() bool {
		if err := testutil.TryReadGetJSON(tests.TestDialClient, url, r2); err != nil {
			return false
		}
		r2.Adjust()
		return reflect.DeepEqual(&response.RegionsInfo{Count: 1, Regions: []response.RegionInfo{*response.NewAPIRegionInfo(r)}}, r2)
	})

	url = fmt.Sprintf("%s/regions/check/%s", urlPrefix, "pending-peer")
	r3 := &response.RegionsInfo{}
	re.NoError(testutil.ReadGetJSON(re, tests.TestDialClient, url, r3))
	r3.Adjust()
	...
	re.NoError(testutil.ReadGetJSON(re, tests.TestDialClient, url, r4))
	r4.Adjust()
	...
	testutil.Eventually(re, func() bool {
		if err := testutil.TryReadGetJSON(tests.TestDialClient, url, r5); err != nil {
			return false
		}
		r5.Adjust()
		return reflect.DeepEqual(&response.RegionsInfo{Count: 1, Regions: []response.RegionInfo{*response.NewAPIRegionInfo(r)}}, r5)
	})
	...
	testutil.Eventually(re, func() bool {
		if err := testutil.TryReadGetJSON(tests.TestDialClient, url, &r6); err != nil {
			return false
		}
	...
	testutil.Eventually(re, func() bool {
		if err := testutil.TryReadGetJSON(tests.TestDialClient, url, &r7); err != nil {
			return false
		}
	...
	testutil.Eventually(re, func() bool {
		if err := testutil.TryReadGetJSON(tests.TestDialClient, url, r8); err != nil {
			return false
		}
		r4.Adjust()
...
func (suite *regionTestSuite) checkRegions(cluster *tests.TestCluster) {
	...
	err := testutil.ReadGetJSON(re, tests.TestDialClient, url, regionsInfo)
	re.NoError(err)
...
func (suite *regionTestSuite) checkStoreRegions(cluster *tests.TestCluster) {
	...
	r4 := &response.RegionsInfo{}
	err := testutil.ReadGetJSON(re, tests.TestDialClient, url, r4)
	re.NoError(err)
	...
	err = testutil.ReadGetJSON(re, tests.TestDialClient, url, r6)
	re.NoError(err)
...
func checkTopRegions(re *require.Assertions, url string, regionIDs []uint64) {
	...
	err := testutil.ReadGetJSON(re, tests.TestDialClient, url, regions)
	re.NoError(err)
...
func (suite *regionTestSuite) checkRegionKey(cluster *tests.TestCluster) {
	...
	err := testutil.ReadGetJSON(re, tests.TestDialClient, url, RegionInfo)
	re.NoError(err)
...
func (suite *regionTestSuite) checkScanRegionByKeys(cluster *tests.TestCluster) {
	...
	err = testutil.CheckGetJSON(tests.TestDialClient, url, nil,
		...
		testutil.StringEqual(re, "\"encoding/hex: invalid byte: U+0069 'i'\"\n"))
	re.NoError(err)
}
...
func (suite *regionTestSuite) checkRegionRangeHoles(cluster *tests.TestCluster) {
	...
	re.NoError(testutil.ReadGetJSON(re, tests.TestDialClient, url, rangeHoles))
	re.Equal([][]string{
