...
func (suite *testRegionSuite) TearDownSuite() {
	suite.cleanup()
}

func (suite *testRegionSuite) TestRegion() {
...
func (suite *testRegionSuite) TestRegionCheck() {
	r := core.NewTestRegionInfo(2, 1, []byte("a"), []byte("b"),
...
func (suite *testRegionSuite) TestRegions() {
	re := suite.Require()
...
func (suite *testRegionSuite) TestStoreRegions() {
	re := suite.Require()
	r1 := core.NewTestRegionInfo(2, 1, []byte("a"), []byte("b"))
...
func (suite *testRegionSuite) TestTop() {
	// Top flow.
	re := suite.Require()
...
func (suite *testRegionSuite) TestTopN() {
	re := suite.Require()
	...
	for n := 0; n <= len(writtenBytes)+1; n++ {
		...
		topN := api.TopNRegions(regions, func(a, b *core.RegionInfo) bool { return a.GetBytesWritten() < b.GetBytesWritten() }, n)
