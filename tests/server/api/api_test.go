...
func TestFollowerRegionResetCacheWithNoForward(t *testing.T) {
	...
	follower.GetServer().DirectlyGetRaftCluster().GetRegionSyncer().StopSyncWithLeader()
	re.False(follower.GetServer().DirectlyGetRaftCluster().GetRegionSyncer().IsRunning())
	...
	storageOnlyRegion := newFollowerStaleRegion(1001)
	...
	staleRegion = newFollowerStaleRegion(1009)
	injectFollowerStaleRegion(re, follower, staleRegion)
	storageOnlyRegion = newFollowerStaleRegion(1019)
	injectFollowerStaleRegionStorageOnly(re, follower, storageOnlyRegion)
