...
func (s *testTsoSuite) TestDeplaySyncTimestamp(c *C) {
	...
	c.Assert(failpoint.Enable("github.com/tikv/pd/v4/server/tso/delaySyncTimestamp", `return(true)`), IsNil)
	...
	failpoint.Disable("github.com/tikv/pd/v4/server/tso/delaySyncTimestamp")
}
...
func (s *testTimeFallBackSuite) SetUpSuite(c *C) {
	s.ctx, s.cancel = context.WithCancel(context.Background())
	c.Assert(failpoint.Enable("github.com/tikv/pd/v4/server/tso/fallBackSync", `return(true)`), IsNil)
	c.Assert(failpoint.Enable("github.com/tikv/pd/v4/server/tso/fallBackUpdate", `return(true)`), IsNil)
	var err error
	...
	svr.Close()
	failpoint.Disable("github.com/tikv/pd/v4/server/tso/fallBackSync")
	failpoint.Disable("github.com/tikv/pd/v4/server/tso/fallBackUpdate")
	err = svr.Run()
	c.Assert(err, IsNil)
	s.cluster.WaitLeader()
}
...
func (s *testFollowerTsoSuite) TestRequest(c *C) {
	c.Assert(failpoint.Enable("github.com/tikv/pd/v4/server/tso/skipRetryGetTS", `return(true)`), IsNil)
	var err error
	cluster, err := tests.NewTestCluster(s.ctx, 2)
	defer cluster.Destroy()
	c.Assert(err, IsNil)
	...
	failpoint.Disable("github.com/tikv/pd/v4/server/tso/skipRetryGetTS")
}
