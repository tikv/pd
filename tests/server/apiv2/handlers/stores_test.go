...
func (s *testStoresAPISuite) TestStoresGet(c *C) {
	...
	for _, s := range s.leaderServer.GetStores() {
		expects = append(expects, handlers.NewMetaStore(s))
	}
	...
	expects1 := []*handlers.MetaStore{
		handlers.NewMetaStore(s.leaderServer.GetStore(1).GetMeta()),
		handlers.NewMetaStore(s.leaderServer.GetStore(4).GetMeta()),
	}
	...
	got = &handlers.StoresInfo{}
	c.Assert(json.Unmarshal(buf, &got), IsNil)
	expects2 := []*handlers.MetaStore{handlers.NewMetaStore(s.leaderServer.GetStore(6).GetMeta())}
...
func (s *testStoresAPISuite) TestStoreGet(c *C) {
	...
	c.Assert(json.Unmarshal(buf, &got), IsNil)
	expects := []*handlers.MetaStore{handlers.NewMetaStore(s.leaderServer.GetStore(1).GetMeta())}
