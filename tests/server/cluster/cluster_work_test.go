...
func (s *clusterWorkerTestSuite) TestSuspectRegions(c *C) {
	...
	highPrioritySchedulingRegions := rc.GetHighPriorityScheduleRegions()
	sort.Slice(highPrioritySchedulingRegions, func(i, j int) bool { return highPrioritySchedulingRegions[i] < highPrioritySchedulingRegions[j] })
	c.Assert(highPrioritySchedulingRegions, DeepEquals, ids)
}
