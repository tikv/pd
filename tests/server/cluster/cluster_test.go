...
func TestLoadClusterInfo(t *testing.T) {
	...
	raftCluster = cluster.NewRaftCluster(ctx, basicCluster, testStorage, syncer.NewRegionSyncer(svr), svr.GetClient(), svr.GetHTTPClient())
...
func TestTransferLeaderForScheduler(t *testing.T) {
	...
	for i := 1; i <= storesNum; i++ {
		...
		resp, err := putStore(grpcPDClient, keypath.ClusterID(), store)
		re.NoError(err)
		re.Equal(pdpb.ErrorType_OK, resp.GetHeader().GetError().GetType())
	}
	// region heartbeat
