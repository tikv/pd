...
func (s *tsoProxyTestSuite) TestTSOProxyClientsWithSameContext() {
	...
	for i := range clientCount {
		conn, err := grpc.NewClient(strings.TrimPrefix(s.backendEndpoints, "http://"), grpc.WithTransportCredentials(insecure.NewCredentials()))
		re.NoError(err)
...
func createTSOStreams(
	...
	for i := range clientCount {
		conn, err := grpc.NewClient(strings.TrimPrefix(backendEndpoints, "http://"), grpc.WithTransportCredentials(insecure.NewCredentials()))
		re.NoError(err)
		grpcPDClient := pdpb.NewPDClient(conn)
