...
func (s *tsoProxyTestSuite) TestTSOProxyClientsWithSameContext() {
	...
	for i := range clientCount {
		conn, err := grpc.Dial(strings.TrimPrefix(s.backendEndpoints, "http://"), grpc.WithTransportCredentials(insecure.NewCredentials())) //nolint:staticcheck // grpc.Dial remains supported throughout grpc 1.x.
		re.NoError(err)
...
func createTSOStreams(
	...
	for i := range clientCount {
		conn, err := grpc.Dial(strings.TrimPrefix(backendEndpoints, "http://"), grpc.WithTransportCredentials(insecure.NewCredentials())) //nolint:staticcheck // grpc.Dial remains supported throughout grpc 1.x.
		re.NoError(err)
		grpcPDClient := pdpb.NewPDClient(conn)
