...
func (suite *tsoServerTestSuite) TestTSOServerStartAndStopNormally() {
	...
	cc, err := grpc.DialContext(suite.ctx, s.GetAddr(), grpc.WithTransportCredentials(insecure.NewCredentials())) //nolint:staticcheck // grpc.DialContext remains supported throughout grpc 1.x.
	re.NoError(err)
	cc.Close()
