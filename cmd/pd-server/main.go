...
func main() {
	...
	rootCmd.Flags().StringP("client-urls", "", config.DefaultClientUrls, "url for client traffic")
	rootCmd.Flags().StringP("advertise-client-urls", "", "", "advertise url for client traffic (default '${client-urls}')")
	rootCmd.Flags().StringP("peer-urls", "", config.DefaultPeerUrls, "url for peer traffic")
...
func NewTSOServiceCommand() *cobra.Command {
	...
	cmd.Flags().StringP("backend-endpoints", "", config.DefaultClientUrls, "url for etcd client")
