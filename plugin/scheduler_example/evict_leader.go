...
func (s *evictLeaderScheduler) Schedule(cluster schedule.Cluster, dryRun bool) ([]*operator.Operator, []plan.Plan) {
	...
	for id, ranges := range s.conf.StoreIDWitRanges {
		region := filter.SelectOneRegion(cluster.RandLeaderRegions(id, ranges), []filter.RegionFilter{pendingFilter, downFilter})
		if region == nil {
			continue
		}
		target := filter.NewCandidates(cluster.GetFollowerStores(region)).
			FilterTarget(cluster.GetOpts(), []filter.Filter{&filter.StoreStateFilter{ActionScope: EvictLeaderName, TransferLeader: true}}).
			RandomPick()
		if target == nil {
			continue
		}
